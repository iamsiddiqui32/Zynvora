  ...
  return (
    <section className="relative h-[88vh] min-h-[560px] w-full overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="animate-float-slow absolute right-[15%] top-[60%] h-56 w-56 rounded-full bg-gold/10 blur-3xl" style={{ animationDelay: "2s" }} />
      </div>
      <Particles count={22} />
          ...
          <div className="mt-8 flex items-center gap-2">
            {slides.map((s, idx) => (
              <button
                ...
                onClick={() => setI(idx)}
                className={`h-1.5 rounded-full transition-all ${idx === i ? "w-10 bg-gold" : "w-5 bg-white/30 hover:bg-white/50"}`}
              />
            ))}
          </div>
        )}
